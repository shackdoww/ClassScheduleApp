...
import sqlite3, os, smtplib, secrets, re
...
def get_current_user():
    if not current_user_id():
        return None
    db = get_db()
    user = db.execute("SELECT id, username, email FROM users WHERE id = ?", (current_user_id(),)).fetchone()
    db.close()
    return user
...
def migrate_users(db):
    ...
    rows = db.execute("SELECT id,email FROM users WHERE username IS NULL OR username='' ORDER BY id").fetchall()
    for row in rows:
...
def init_db():
    ...
    # Universal app: new installations/users start with an empty schedule.
    db.commit(); db.close()


def claim_legacy_data(uid):
    """Migrate legacy notification data without claiming orphaned class schedules."""
    db=get_db()
...
def ensure_user_settings(uid):
    db=get_db()
    db.execute("INSERT OR IGNORE INTO notification_settings(user_id,enabled,email,minutes_before,second_minutes_before,class_start_enabled,morning_digest_enabled,commute_minutes) VALUES(?,0,'',30,0,0,0,0)",(uid,))
    db.execute("UPDATE users SET share_token=COALESCE(share_token,?) WHERE id=?",(secrets.token_urlsafe(18),uid))
    db.commit(); db.close()
...
def send_email(recipient,subject,body):
    if not recipient or not EMAIL_ADDRESS or not EMAIL_APP_PASSWORD: return False
    try:
...
def login():
    ...
    if request.method=="POST":
        username=request.form.get("username","").strip(); password=request.form.get("password",""); db=get_db(); u=db.execute("SELECT * FROM users WHERE username=? COLLATE NOCASE",(username,)).fetchone(); db.close()
        if not u or not check_password_hash(u["password_hash"],password): flash("Invalid username or password.","error")
...
def register():
    ...
    if request.method=="POST":
        ...
        if not re.fullmatch(r"[A-Za-z0-9_.-]{3,30}",username): flash("Username must be 3–30 characters and use only letters, numbers, dots, underscores, or hyphens.","error")
            ...
            db=get_db()
            try:
                placeholder_email=f"{username.lower()}@local.invalid"
                cur=db.execute("INSERT INTO users(username,email,password_hash,created_at,share_token) VALUES(?,?,?,?,?)",(username,placeholder_email,generate_password_hash(password),datetime.now(PHILIPPINES).isoformat(),secrets.token_urlsafe(18))); new_uid=cur.lastrowid; db.commit()
            except sqlite3.IntegrityError: db.close(); flash("That username is already taken.","error"); return render_template("register.html")
...
def save_settings():
    ...
    if email and not re.fullmatch(r"[^\s@]+@[^\s@]+\.[^\s@]+",email): flash("Please enter a valid notification email address.","error"); return redirect(url_for("settings"))
    def n(name,default=0):
        try: return int(request.form.get(name,default))
        except (ValueError,TypeError): return default
    ensure_user_settings(current_user_id()); db=get_db(); db.execute("UPDATE notification_settings SET enabled=?,email=?,minutes_before=?,second_minutes_before=?,class_start_enabled=?,morning_digest_enabled=?,commute_minutes=? WHERE user_id=?",(1 if request.form.get("enabled") and email else 0,email,max(1,min(n("minutes_before",30),1440)),max(0,min(n("second_minutes_before",0),1440)),1 if request.form.get("class_start_enabled") and email else 0,1 if request.form.get("morning_digest_enabled") and email else 0,max(0,min(n("commute_minutes",0),180)),current_user_id())); db.commit(); db.close(); flash("Notification settings saved.","success"); return redirect(url_for("settings"))
...
def test_email():
    ...
    if not s or not s["enabled"] or not s["email"]: flash("Enable email notifications and provide an email address first.","error"); return redirect(url_for("settings"))
