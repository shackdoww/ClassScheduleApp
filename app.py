...
def migrate_notification_settings(db):
    """Convert the original single-row settings table into a multi-user table."""
    ...
    sql = (table["sql"] or "").upper()
    columns = [r["name"] for r in db.execute("PRAGMA table_info(notification_settings)").fetchall()]
    if "CHECK (ID = 1)" in sql.replace("  ", " ") or ("user_id" not in columns):
        old_rows = db.execute("SELECT enabled, email, minutes_before FROM notification_settings").fetchall()
        ...
        """)
        # Legacy settings are intentionally kept temporarily in the legacy table.
        # claim_legacy_data() assigns them to the first account.
        return
...
def claim_legacy_data(user_id):
    ...
    # If this database came from the original app, transfer its one settings row.
    ...
    if legacy:
        ...
        if old:
            email = old["email"] if valid_ndmu_email(old["email"] or "") else user["email"]
            db.execute("INSERT OR IGNORE INTO notification_settings (user_id,enabled,email,minutes_before) VALUES (?,?,?,?,?)", (user_id,old["enabled"],email,old["minutes_before"]))
            db.execute("DROP TABLE notification_settings_legacy")
    db.execute("INSERT OR IGNORE INTO notification_settings (user_id,enabled,email,minutes_before) VALUES (?,0,?,30)", (user_id,user["email"]))
    db.commit()
    db.close()
...
def get_next_class(user_id):
    ...
    db = get_db(); classes = db.execute("SELECT * FROM classes WHERE user_id=? ORDER BY day_of_week,start_time", (user_id,)).fetchall(); db.close()
    candidates=[]
    for c in classes:
        ...
        if dt>now: candidates.append((dt,c))
...
def check_for_reminders():
    now=datetime.now(PHILIPPINES); db=get_db()
    ...
    for s in settings_rows:
        for offset in range(2):
            ...
            classes=db.execute("SELECT * FROM classes WHERE user_id=? AND day_of_week=? ORDER BY start_time",(s["user_id"],date.weekday())).fetchall()
            for c in classes:
                start=datetime.strptime(c["start_time"],"%H:%M").time(); dt=datetime.combine(date,start).replace(tzinfo=PHILIPPINES)
                reminder=dt-timedelta(minutes=s["minutes_before"]); delta=(reminder-now).total_seconds()
                if -90<delta<=0 and not db.execute("SELECT id FROM sent_reminders WHERE class_id=? AND class_date=?",(c["id"],date.isoformat())).fetchone():
                    body=f"Class Schedule Reminder\n\nSubject: {c['subject']}\n\nTime: {c['start_time']} - {c['end_time']}\n\nRoom: {c['room']}\n\nThis class starts in approximately {s['minutes_before']} minutes."
                    if send_email(s["email"],f"Class Reminder: {c['subject']}",body):
                        try: db.execute("INSERT INTO sent_reminders (class_id,class_date,sent_at) VALUES (?,?,?)",(c["id"],date.isoformat(),now.isoformat())); db.commit()
...
def login():
    ...
    if request.method=="POST":
        ...
        if not valid_ndmu_email(email) or not user or not check_password_hash(user["password_hash"],password):
            flash("Use a valid @ndmu.edu.ph account and password.","error"); return render_template("login.html")
        session.clear(); session["user_id"]=user["id"]; return redirect(url_for("home"))
...
def register():
    ...
    if request.method=="POST":
        ...
        if not valid_ndmu_email(email): flash("Registration is limited to @ndmu.edu.ph institutional emails.","error")
            ...
            db=get_db()
            try:
                cur=db.execute("INSERT INTO users (email,password_hash,created_at) VALUES (?,?,?)",(email,generate_password_hash(password),datetime.now(PHILIPPINES).isoformat())); uid=cur.lastrowid; db.commit()
            except sqlite3.IntegrityError:
                db.close(); flash("That institutional email already has an account.","error"); return render_template("register.html")
            db.close(); claim_legacy_data(uid); session.clear(); session["user_id"]=uid; return redirect(url_for("home"))
...
@login_required
def home():
    uid=current_user_id(); now=datetime.now(PHILIPPINES); days=["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
    db=get_db(); classes=db.execute("SELECT * FROM classes WHERE user_id=? ORDER BY day_of_week,start_time",(uid,)).fetchall(); db.close()
    schedule={d:[dict(c) for c in classes if c["day_of_week"]==i] for i,d in enumerate(days)}
    return render_template("index.html",schedule=schedule,days=days,next_class=get_next_class(uid),today_name=days[now.weekday()],current_time=now.strftime("%H:%M:%S"),user=get_current_user())


@app.route("/settings")
@login_required
def settings():
    uid=current_user_id(); ensure_user_settings(uid); db=get_db(); settings=db.execute("SELECT * FROM notification_settings WHERE user_id=?",(uid,)).fetchone(); history=db.execute("SELECT s.class_date,s.sent_at,c.subject FROM sent_reminders s JOIN classes c ON c.id=s.class_id WHERE c.user_id=? ORDER BY s.sent_at DESC LIMIT 20",(uid,)).fetchall(); db.close()
...
def save_settings():
    ...
    try: minutes=max(1,min(int(request.form.get("minutes_before",30)),1440))
    except ValueError: minutes=30
    enabled=1 if request.form.get("enabled")=="on" else 0; db=get_db(); db.execute("UPDATE notification_settings SET enabled=?,email=?,minutes_before=? WHERE user_id=?",(enabled,email,minutes,current_user_id())); db.commit(); db.close(); flash("Notification settings saved.","success"); return redirect(url_for("settings"))
...
def add_class():
    ...
    if not days or not subject or not start or not end: flash("Please select at least one day and complete the class details.","error"); return redirect(url_for("home")+"#add")
    if end<=start: flash("End time must be later than start time.","error"); return redirect(url_for("home")+"#add")
    db=get_db()
    for day in days:
        try:
            day=int(day)
            if 0<=day<=6: db.execute("INSERT INTO classes (user_id,day_of_week,subject,start_time,end_time,room) VALUES (?,?,?,?,?,?)",(current_user_id(),day,subject,start,end,room))
        except ValueError: pass
    db.commit(); db.close(); return redirect(url_for("home"))


@app.route("/delete/<int:class_id>",methods=["POST"])
@login_required
def delete_class(class_id):
    db=get_db(); db.execute("DELETE FROM classes WHERE id=? AND user_id=?",(class_id,current_user_id())); db.commit(); db.close(); return redirect(url_for("home"))


@app.route("/edit/<int:class_id>",methods=["GET","POST"])
@login_required
def edit_class(class_id):
    db=get_db(); c=db.execute("SELECT * FROM classes WHERE id=? AND user_id=?",(class_id,current_user_id())).fetchone()
    if not c: db.close(); return redirect(url_for("home"))
    if request.method=="POST":
        try: day=int(request.form["day_of_week"])
        except ValueError: day=-1
        subject=request.form.get("subject","").strip(); start=request.form.get("start_time",""); end=request.form.get("end_time",""); room=request.form.get("room","").strip() or "NOT SET"
        if day not in range(7) or not subject or end<=start: db.close(); flash("Please enter valid class details.","error"); return redirect(url_for("edit_class",class_id=class_id))
        db.execute("UPDATE classes SET day_of_week=?,subject=?,start_time=?,end_time=?,room=? WHERE id=? AND user_id=?",(day,subject,start,end,room,class_id,current_user_id())); db.commit(); db.close(); return redirect(url_for("home"))
    db.close(); return render_template("edit.html",class_item=c,user=get_current_user())
...
def test_email():
    ...
    if not valid_ndmu_email(s["email"]): flash("Set a valid @ndmu.edu.ph notification email first.","error"); return redirect(url_for("settings"))
    ok=send_email(s["email"],"📚 Class Schedule App - Test Email","Hello!\n\nThis is a test email from your Class Schedule App.\n\nYour institutional email notifications are working correctly.\n\n— Class Schedule App")
...
if __name__=="__main__":
    init_db(); scheduler=BackgroundScheduler(timezone=PHILIPPINES); scheduler.add_job(check_for_reminders,"interval",minutes=1,id="class_reminder_checker",replace_existing=True); scheduler.start(); print("Notification scheduler started."); app.run(host="0.0.0.0",port=5000,debug=True,use_reloader=False)
